...
func TestEncodeBaseInstructions(t *testing.T) {
	...
	ctrl1 := BaseInstruction{ // BEQ [r5 + 0x200]
		OpType:   Control,               // 11
		RMem:     5,                     // 00101
		CtrlFlag: Conditions["eq"].Flag, // BEQ (Flag 0001)
		CtrlMode:  Conditions["eq"].Mode, // 000
	...
	ctrl2 := BaseInstruction{ // BOF [r6 + 0x400]
		OpType:   Control,               // 11
		RMem:     6,                     // 00110
		CtrlFlag: Conditions["of"].Flag, // BOF (Flag 0100)
		CtrlMode:  Conditions["of"].Mode, // 100
	...
	ctrl3 := BaseInstruction{ // BNF r10
		OpType:   Control,               // 11
		RMem:     10,                    // 01010 (r10)
		CtrlFlag: Conditions["nf"].Flag, // BNF (Flag 0100)
		CtrlMode:  Conditions["nf"].Mode, // 000
	}
...
func TestDecodeBaseInstruction(t *testing.T) {
	...
	var test = []struct {
		...
		},
		{
			...
			expected: BaseInstruction{
				...
				RMem:     5,                     // 00101 (r5)
				CtrlFlag: Conditions["eq"].Flag, // BEQ (Flag 0001)
				CtrlMode:  Conditions["eq"].Mode, // 000 (Normal mode)
				Imm:      0x200,                 // 0000 0010 0000 0000
			},
		},
		{
			...
			expected: BaseInstruction{
				...
				RMem:     6,                     // 00110 (r6)
				CtrlFlag: Conditions["of"].Flag, // BOF (Flag 0100)
				CtrlMode:  Conditions["of"].Mode, // 100 (Normal mode)
				Imm:      0x400,                 // 0000 0100 0000 0000
			},
		},
		{
			...
			expected: BaseInstruction{
				...
				RMem:     10,                    // 01010 (r10)
				CtrlFlag: Conditions["nf"].Flag, // BNF (Flag 0100)
				CtrlMode:  Conditions["nf"].Mode, // 000 (Normal mode)
	...
	for num, testCase := range test {
		...
		if inst.OpType != testCase.expected.OpType ||
			...
			inst.CtrlFlag != testCase.expected.CtrlFlag ||
			inst.CtrlMode != testCase.expected.CtrlMode ||
			inst.MemMode != testCase.expected.MemMode ||
